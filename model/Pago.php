...
define ("get_all_pagos", 'SELECT *  FROM pago order by id DESC');
define ("get_all_bookings", 'SELECT b.codigo, b.id from booking b');
define ("get_pago_by_id", 'SELECT *  FROM pago where id=%');
...
//            return "No se encontraron pagos.";
    ...
    public static function update($id,$ruta_comprobante, $fecha, $id_booking) {
        $values = array();
        $values[0] = $ruta_comprobante;
        $values[1] = $fecha;
        $values[2] = $id_booking;
